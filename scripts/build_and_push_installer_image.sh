...
IMAGE_NAME="plugin-installer-job" # Changed image name
...
# Run build using the plugin-installer-job Dockerfile and context
# Explicitly specify the target platform for Cloud Run compatibility
docker build --no-cache --platform linux/amd64 -t "${IMAGE_URL}" -f "${PROJECT_ROOT}/src/plugin-installer-job/Dockerfile" "${PROJECT_ROOT}/src/plugin-installer-job" # Changed Dockerfile path and context
